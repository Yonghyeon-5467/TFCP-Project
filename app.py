...
def process_frame(img):
    ...
    for box in ai_raw_boxes:
        ...
        # 신호 검증 복구
        if np.sum(cv2.inRange(roi_hsv, np.array([0, 35, 35]), np.array([60, 255, 255]))) + np.sum(cv2.inRange(roi_hsv, np.array([80, 30, 30]), np.array([165, 255, 255]))) > 100:
    ...
    for i, (box, method) in enumerate(combined_boxes):
        ...
        # [복구] 색상 범위 (Orange 0-60, Cyan 80-165)
        ...
        if p_count < 50 and method == "AI":
            ...
            # 백업 모드거나 p_count가 적으면 전체를 후보로 (백업 모드 시)
            ...
            cv_color = (255, 255, 0) if status == "CONTAMINATED" else (0, 255, 0) # Cyan or Green (BGR)
...
# --- [4] UI: 관리자 페이지 (함수로 분리하여 스코프 오류 방지) ---
        ...
        if os.path.exists(img_path):
            ...
            if particles:
                ...
                with st.expander("➕ 수동 영역 지정 (Manual Selection)", expanded=False):
                    ...
                    st.image(standardize_image_size(preview, 800, 600), caption="미리보기", width=800)
