...
print(f"[Setup] Data path: {BIGDATA_PATH}")

# --- [1. app.py 생성 (드래그 기능 포함)] ---
app_code = """
...
@st.cache_resource
def load_model():
    if os.path.exists('best.pt'): return YOLO('best.pt')
    return None
model = load_model()
...
def detect_particles_heuristically(img):
    ...
    for cnt in contours:
        if cv2.contourArea(cnt) > 3000:
            ...
            if 0.2 < float(w)/h < 5.0: found_boxes.append(FakeBox([x, y, x+w, y+h]))
    return found_boxes
...
def render_admin_page():
    st.title("🗂️ 연구 데이터 관리 센터 (Canvas)")
    ...
    if os.path.exists(img_path):
        ...
        st.write(f"### {data['timestamp']} (Area: {len(particles)}개)")
        ...
        canvas_result = st_canvas(
            ...
            drawing_mode="rect", initial_drawing=initial_drawing, key=f"canvas_{data['timestamp']}"
        )
            ...
            with st.form("update_form"):
                ...
                for i, obj in enumerate(current_objects):
                    ...
                    with cols[i % 2]:
                        ...
                        try: idx = ["SAFE", "CONTAMINATED", "RECHECK REQUIRED"].index(prev_status)
                        except: idx = 1
...
if mode == "관리자 모드":
    ...
    if img_file:
        ...
        with col1: st.image(cv2.cvtColor(result_img, cv2.COLOR_BGR2RGB), caption="분석 완료", use_container_width=True)
        with col2:
            if reports:
                for r in reports:
...
# --- [2. requirements.txt 생성 (Canvas 라이브러리 추가)] ---
req_code = """
streamlit
ultralytics
...
print("[SUCCESS] app.py & requirements.txt 업데이트 완료!")
